...
# ejecutar de la siguiente forma: ./twitter_job_general.sh 2016-04-03
...
if [ -z "$1" ]
	then
		./kitchen.sh -file:$PATH_JOB/twitter_job_gral.kjb ${desde} -level:$LOG_LEVEL -log=$PATH_LOG/twitter_job_gral.log
	else
		./kitchen.sh -file:$PATH_JOB/twitter_job_gral.kjb $1 -level:$LOG_LEVEL -log=$PATH_LOG/twitter_job_gral.log
fi
