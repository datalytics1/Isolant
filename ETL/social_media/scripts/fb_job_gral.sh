...
# ejecutar de la siguiente forma: ./fb_job_general.sh 2016-04-03
...
if [ -z "$1" ]
	then
		./kitchen.sh -file:$PATH_JOB/fb_job_gral.kjb ${desde} -level:$LOG_LEVEL -log=$PATH_LOG/fb_job_gral.log
	else
		./kitchen.sh -file:$PATH_JOB/fb_job_gral.kjb $1 -level:$LOG_LEVEL -log=$PATH_LOG/fb_job_gral.log
fi
