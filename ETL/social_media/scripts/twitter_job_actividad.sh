...
if [ -z "$1" ]
        then
                ./kitchen.sh -file:$PATH_JOB/job_tw_actividad.kjb ${desde} -level:$LOG_LEVEL -log=$PATH_LOG/job_tw_actividad.log
        else
                ./kitchen.sh -file:$PATH_JOB/job_tw_actividad.kjb $1 -level:$LOG_LEVEL -log=$PATH_LOG/job_tw_actividad.log
fi
